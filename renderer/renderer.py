...
class Renderer(ABC):
    ...
    def _render_center_text(self, draw, text, y, font=None):
        x = center_text(self.font_mini.getsize(text)[0], self._get_center())
    ...
    def _render_right_text(self, draw, text, y, font=None):
        x = right_text(self.font_mini.getsize(text)[0], self.screen_width)
        self._render_text(draw, text, x, y, font)
    ...
    def _render_text(self, draw, text, x, y, font=None):
        ...
        draw.multiline_text((x, y), text, fill=self.text_color,
                            font=font,
                            align="center")
